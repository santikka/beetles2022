...
repros <- bind_rows(repro1, repro2) |>
    ...
    rename(total = nr_eggs_10d,
           batches = nr_batches_10d,
           hatched = nr_hatching_10d) |>
    mutate(mother = match(Mother_ID, gens$ID, incomparables = NA),
           rate = total / batches,
...
repro_const <- generate_repro_consts(
    x = repros,
    cov_inits = list(repro_cov_init_rate,
                     repro_cov_init_kl,

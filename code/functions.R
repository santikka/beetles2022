...
generate_consts <- function(x, s, cov_inits = list(),
    ...
    sp_basis_c <- bs(trt[!ti], knots = seq(0.0, 8.0, by = 0.5), degree = 3,
                     Boundary.knots = c(0.0, 8.0))
    sp_basis_t <- bs(trt[ti], knots = seq(0.0, 6.0, by = 0.5), degree = 3,
    ...
    named_list(N, N_c, N_t, N_fam, N_fam1, N_fam2,
...
generate_repro_consts <- function(x, cov_inits = list()) {
    ...
    named_list(N, B, gen, gen2, age4, age5, trt, pa_trt, ti, bm,
...
mcmc_ess <- function(x, filter) {
    if (is.list(x)) {
        ...
        x |> mcmc_clean() |> coda::effectiveSize()
    }
}
...
family_effects <- function(samples, idx, fun = exp, slice_vec) {
    ...
    out <- samples |>
        ...
        mutate(family = gsub(pattern, "\\1", family, perl = TRUE) |> as.integer(),
...
survival_curve <- function(samples, const, fun = expit, prob = FALSE) {
    ...
    sp_basis_c <- t(bs(trt_plot, knots = attr(const$sp_basis_c, "knots"), degree = 3,
                     Boundary.knots = attr(const$sp_basis_c, "Boundary.knots")))
    sp_basis_t <- t(bs(trt_plot, knots = attr(const$sp_basis_t, "knots"), degree = 3,
...
curve_plot <- function(curve, fun = c("identity", "logit"), breaks = "", y_label = "") {
    ...
    curve |> ggplot(aes(x = treatment, y = center, colour = group)) +
        ...
        scale_colour_manual(name = "Transgenerational\ntreatment",
                            values = c("black", "red", "blue"),
                            labels = c("Control", "Treated (1.0 \u00b5g/ml)", "Treated (8.0 \u00b5g/ml)")) +
        scale_y_continuous(trans = fun,
                           breaks = breaks) +
        geom_line() +
        labs(x = "Within-generational treatment (\u00b5g/ml)",
             y = y_label) +
...
survival_predictive_plot <- function(pred, surv_var, data, y_label = "") {
    pred |>
        ...
        labs(x = "Within-generational treatment / Transgenerational treatment (\u00b5g/ml)",
             y = y_label)
}
...
generational_differences <- function(samples, transfun = identity, comparefun = `-`, idx) {
    ...
    summarise(mean = mean(effect),
              sign = sign(mean),
              EP = if (sign == 1) mean(effect > transfun(0))
                   else mean(effect < transfun(0)))
}

body_mass_curve <- function(samples, const, idx, data) {
    trt_plot <- seq(0, 6, by = 0.1)
    sp_basis_c <- t(bs(trt_plot, knots = attr(const$sp_basis_c, "knots"), degree = 3,
                     Boundary.knots = attr(const$sp_basis_c, "Boundary.knots")))
    sp_basis_t <- t(bs(trt_plot, knots = attr(const$sp_basis_t, "knots"), degree = 3,
                     Boundary.knots = attr(const$sp_basis_t, "Boundary.knots")))
    bm_spline_c <- samples[,paste0("sp_c[", 1:const$S_c, ", ", idx, "]")] %*% sp_basis_c
    bm_spline_t <- samples[,paste0("sp_t[", 1:const$S_t, ", ", idx, "]")] %*% sp_basis_t
    bm_transgen <- samples[,paste0("beta[1, ", idx, "]")]
    bm_ad <- 0
    if (idx == 2) {
        bm_ad <- samples[,"beta[4, 2]"] * mean(data$bm |> pull(var = 1), na.rm = TRUE)
    }
    bind_rows(
        summarise_curve(bm_spline_c + bm_ad, group = "1",
                        transfun = \(x) x^2, centerfun = mean),
        summarise_curve(bm_spline_t + bm_ad + bm_transgen * 1.00, group = "2",
                        transfun = \(x) x^2, centerfun = mean),
        summarise_curve(bm_spline_t + bm_ad + bm_transgen * 8.00, group = "3",
                        transfun = \(x) x^2, centerfun = mean)
    )
}

body_mass_hormetic  <- function(samples, const, idx) {
    trt_plot <- seq(0, 6, by = 0.1)
    sp_basis_c <- t(bs(trt_plot, knots = attr(const$sp_basis_c, "knots"), degree = 3,
                     Boundary.knots = attr(const$sp_basis_c, "Boundary.knots")))
    sp_basis_t <- t(bs(trt_plot, knots = attr(const$sp_basis_t, "knots"), degree = 3,
    ...
    bm_spline_c <- samples[,paste0("sp_c[", 1:const$S_c, ", ", idx, "]")] %*% sp_basis_c
    bm_spline_t <- samples[,paste0("sp_t[", 1:const$S_t, ", ", idx, "]")] %*% sp_basis_t
    (bm_spline_t - bm_spline_c) |>
    as.vector() |>
    as.data.frame() |>
    `colnames<-`("effect") |>
    summarise(mean = mean(effect), sign = sign(mean), EP = mean(sign * effect > 0))
}
...
restore_families <- function(mcmc_const, mcmc_samples, nvars = 2) {
    ...
    if (nvars == 2) {
        colnames(f) <- c(paste0("f[", 1:mcmc_const$N_fam, ", ", 1, "]"),
...
surv_table <- function(x, idx, suffix) {
    ...
    mcmc_summary(x, filter = c("f", "sp", "log_sigma"),
...
repro_table_rate <- function(x, idx) {
    ...
    mcmc_summary(x, filter = c("phi", "sigma", "theta"),
...
repro_table_hatching <- function(x, idx) {
    ...
    mcmc_summary(x, filter = c("phi", "lambda", "kappa"),
                 transforms = transform_repro, effects = effects_repro) |>
        select(!Median) |>

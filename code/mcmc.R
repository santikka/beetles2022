...
surv_mcmc <- function(seed, mcmc_const, mcmc_data,
    ...
    conf <- configureMCMC(mod, monitors = c("beta", "f", "f_raw", "rho_f",
    ...
    conf$addSampler(target = c("log_sigma_sp_c", paste0("sp_c[1:", mcmc_const$S_c, "]")),
                    type = "RW_block", control = mcmc_const$control[[1]])
    conf$addSampler(target = c("log_sigma_sp_t", paste0("sp_t[1:", mcmc_const$S_t, "]")),
...
bm_mcmc <- function(seed = 0, mcmc_const, mcmc_data,
    ...
    mod <- nimbleModel(
        ...
        inits = list(
            beta = rbind(rnorm(2, c(11, 5.5), sd = 0.25),
    ...
    conf$addSampler(target = paste0("beta[", 1:6, ", 1]"),
                    type = "RW_block", control = mcmc_const$control[[1]])
    conf$addSampler(target = paste0("beta[", 2:6, ", 2]"),
                    type = "RW_block", control = mcmc_const$control[[2]])
    Rmcmc <- buildMCMC(conf)
    Cmod <- compileNimble(mod)
    if (run) {
...
repro_mcmc <- function(seed, mcmc_const, mcmc_data,
    ...
    conf$addSampler(target = paste0("beta[", 1:9, ", 1]"),
                    type = "RW_block", control = mcmc_const$control[[1]])
    conf$addSampler(target = c("kappa[1]", "kappa[2]", "lambda[1]", "lambda[2]"),
                    type = "RW_block", control = mcmc_const$control[[2]])
    conf$addSampler(target = paste0("beta[", 1:9, ", 2]"),
                    type = "RW_block", control = mcmc_const$control[[3]])
    conf$addSampler(target = paste0("theta[", 1:9, "]"),

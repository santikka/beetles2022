...
gg_surv_family_24h_pairs <- surv_family_24h |>
  ...
  labs(
    x = "Log-mean of 1st gen. family effect",
    y = "Log-mean of 2nd gen. family effect"
  ) +
  new_scale_color() +
  geom_point(
    data = data.frame(extinct = c("a", "b", "c", "d"), pa_rank = 1),
...
gg_repro_curve_rate <- curve_plot(
  curve = repro_curve_rate,
  y_label = "Mean number of eggs per batch"
)

gg_repro_curve_hatching <- curve_plot(
  curve = repro_curve_hatching,
  y_label = "Mean hatching probability"
)
...
save(
  gg_surv_family_24h, gg_surv_family_24h_pairs, gg_surv_curve_24h, gg_surv_benefit_24h,

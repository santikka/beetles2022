...
# Body mass (females)
bm_pp_ad_f <- bm_pp_samples_f |> as.data.frame() |> select(matches("bm\\[.+, 1\\]"))
...
bm_pp_10d_f <- bm_pp_samples_f |> as.data.frame() |> select(matches("bm\\[.+, 2\\]"))
...
# Body mass (males)
bm_pp_ad_m <- bm_pp_samples_m |> as.data.frame() |> select(matches("bm\\[.+, 1\\]"))
...
bm_pp_10d_m <- bm_pp_samples_m |> as.data.frame() |> select(matches("bm\\[.+, 2\\]"))
...
save(surv_pred_24h, surv_gens_24h, surv_family_24h, surv_curve_24h, surv_benefit_24h,
     surv_pred_la, surv_gens_la, surv_family_la, surv_curve_la, surv_benefit_la,
     bm_pvals_ad_f, bm_gens_ad_f, bm_family_ad_f,
     bm_pvals_10d_f, bm_gens_10d_f, bm_family_10d_f,
